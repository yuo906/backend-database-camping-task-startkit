...
('透明人', 'opacity@hexschooltest.io' , 'USER');
...
(SELECT "id", 2 FROM "USER" WHERE email = 'lee2000@hexschooltest.io');

INSERT INTO "COACH"(user_id, experience_years)
SELECT "id", 2 FROM "USER" WHERE email = 'muscle@hexschooltest.io';

INSERT INTO "COACH"(user_id, experience_years)
SELECT "id", 2 FROM "USER" WHERE email = 'starplatinum@hexschooltest.io';
...
select "COURSE_BOOKING".user_id, count("COURSE_BOOKING".user_id) as total 
from "COURSE_BOOKING" 
...
inner join "USER" on "COURSE_BOOKING".user_id = "USER".id 
where "USER".email = 'wXlTq@hexschooltest.io'
and "COURSE_BOOKING".join_at notnull 
group by "COURSE_BOOKING".user_id;
